...
// coconet: modified from llvm::SmallVector.
...
namespace std {
  ...
  template<typename T>
  inline void
  swap(coconet::SmallVectorImpl<T> &LHS, coconet::SmallVectorImpl<T> &RHS) {
    LHS.swap(RHS);
  }
  ...
  template<typename T, unsigned N>
  inline void
  swap(coconet::SmallVector<T, N> &LHS, coconet::SmallVector<T, N> &RHS) {
    LHS.swap(RHS);
  }
